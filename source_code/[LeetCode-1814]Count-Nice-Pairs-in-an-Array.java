...
***/
//version-1: Math + HashMap + counting
class Solution {
    ...
    public int countNicePairs(int[] nums) {
        ...
        int count = 0;
        for (int num : nums) {
            int diff = num - getReverse(num);
            int repeatCount = map.getOrDefault(diff, 0);

            count = (count + repeatCount) % MOD;

            map.put(diff,repeatCount +1);            
        }
