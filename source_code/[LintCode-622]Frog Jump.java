...
A frog is crossing the river, the river is divided into units, and each unit may exist or not exist a stone. The frog can jump on the stone, but it must not jump into the water.
Given a list of stones' position(in units) in sorted ascending order, determind if the frog is able to cross the river by landing the last stone. Initially, the frog is on the first position and the first jump must be 1 unit.
If the frog's last jump was k unit, then its next jump must be either k -1, k, or k + 1 units. Note that frog can only jump in forward direction.
