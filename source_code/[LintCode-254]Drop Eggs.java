...
Link: https://www.lintcode.com/problem/254/
***/
...
public class Solution {
    ...
    public int dropEggs(int n) {
        ...
        // state
        int[][] dp = new int[n + 1][EGG_COUNT + 1];
        // initialize
        for (int i = 1; i <= n; i++) {
...
两种解法：DP和math
    DP：
        设f（n，k）为n蛋k层最优解。从第i层丢，碎了的话还要试 f（n-1，i-1）次，不碎要试f（n，k-i）次。
        所以任意i的策略是 f(n,k) = 1 + max(f(n-1, i-1), f(n, k-i)). 
        求最优解： f(n,k) = min(1 + max(f(n-1, i-1), f(n, k-i))) for any i in {1..n}.
